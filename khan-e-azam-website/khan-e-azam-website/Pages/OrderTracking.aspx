...
<asp:Content ID="BodyContent" ContentPlaceHolderID="MainContent" runat="server">
	<style>
		...
		.kea-tracker-step::before { content:''; position:absolute; top:22px; left:calc(-50% + 22px); right:calc(50% + 22px); height:3px; background:var(--dark-border); z-index:0; }
		.kea-tracker-step:first-child::before { display:none; }
		.kea-step-icon { width:44px; height:44px; border-radius:50%; background:var(--dark-1); display:flex; align-items:center; justify-content:center; font-size:18px; color:var(--muted-text); z-index:1; transition:all .3s; border:3px solid var(--dark-card); box-shadow:0 2px 8px rgba(0,0,0,.3); }
		.kea-step-label { font-size:11px; font-weight:600; color:var(--muted-text); margin-top:8px; text-align:center; line-height:1.3; text-transform:uppercase; letter-spacing:.5px; }
		...
		.kea-tracker-step.active .kea-step-icon { background:var(--primary); color:#fff; box-shadow:0 0 0 6px var(--rgba-primary-1); }
		...
		.kea-info-row { display:flex; padding:10px 0; border-bottom:1px solid var(--dark-border); }
		.kea-info-row:last-child { border-bottom:none; }
		.kea-info-label { min-width:140px; font-weight:600; font-size:13px; color:var(--muted-text); }
		.kea-info-value { font-size:14px; color:var(--light-text); }
		...
		.kea-info-row { display:flex; flex-wrap:wrap; gap:6px; padding:10px 0; border-bottom:1px solid var(--dark-border); }
		.kea-info-label { width:100%; min-width:auto; font-weight:600; font-size:13px; color:var(--muted-text); }
		.kea-info-value { width:100%; font-size:14px; color:var(--light-text); }
		.kea-item-row { display:flex; flex-wrap:wrap; justify-content:space-between; gap:10px; align-items:flex-start; padding:10px 0; border-bottom:1px solid var(--dark-border); }
		...
		.kea-track-card { border-radius:14px; box-shadow:0 4px 24px rgba(0,0,0,.3); background:var(--dark-card); border:1px solid var(--dark-border); }

		@media (max-width: 768px) {
			.kea-track-card { padding:24px !important; }
	...
	<section class="kea-stars" style="padding:60px 0 80px; background-color:var(--dark-1);">
		<div class="container" style="max-width:760px;">
			...
			<asp:Panel ID="pnlSearch" runat="server">
				<div class="kea-track-card" style="padding:36px;text-align:center;">
					<i class="fa-solid fa-magnifying-glass" style="font-size:2.5rem;color:var(--primary);margin-bottom:16px;display:block;"></i>
					<h4 style="margin-bottom:8px;color:var(--light-text);">Track Your Order</h4>
					<p style="color:var(--muted-text);margin-bottom:24px;">Enter your Order ID below to check the status</p>
					<div class="search-input-group" style="display:flex;gap:10px;max-width:420px;margin:0 auto;">
						<asp:TextBox ID="txtSearchId" runat="server" CssClass="h-[50px] py-[10px] px-5 w-full text-[15px] rounded-[6px] focus:border-primary duration-500" placeholder="Order ID (e.g. 42)" style="border:1px solid var(--dark-border);background:var(--dark-1);color:var(--light-text);flex:1;" />
			...
			<asp:Panel ID="pnlOrder" runat="server" Visible="false">
				...
				<!-- Order ID Banner -->
				<div style="background:var(--rgba-primary-1);border:2px solid var(--primary);border-radius:14px;padding:20px 24px;margin-bottom:20px;text-align:center;">
					<p style="margin:0 0 4px;font-size:12px;color:var(--muted-text);text-transform:uppercase;letter-spacing:1px;">Your Order ID</p>
					<div style="font-size:2.4rem;font-weight:800;color:var(--primary);line-height:1;">#<asp:Label ID="lblOrderId" runat="server" /></div>
					<p style="margin:6px 0 0;font-size:12px;color:var(--muted-text);">Save this number &mdash; use it to track your order anytime</p>
				</div>

				<!-- Status Timeline -->
				<div class="kea-track-card" style="padding:32px;margin-bottom:20px;">
					<div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:24px;">
						<h5 style="margin:0;color:var(--light-text);">Order Status</h5>
				...
				<!-- Order Info -->
				<div class="kea-track-card" style="padding:28px;margin-bottom:20px;">
					<h6 style="margin-bottom:16px;font-size:16px;border-bottom:2px solid var(--dark-border);padding-bottom:10px;color:var(--light-text);">Order Details</h6>
					...
					<div class="kea-info-row"><span class="kea-info-label">Order Type</span><span class="kea-info-value"><asp:Label ID="lblOrderType" runat="server" /></span></div>
				...
				<!-- Order Items -->
				<div class="kea-track-card" style="padding:28px;">
					<h6 style="margin-bottom:16px;font-size:16px;border-bottom:2px solid var(--dark-border);padding-bottom:10px;color:var(--light-text);">Ordered Items</h6>
					<asp:Repeater ID="rptItems" runat="server">
						<ItemTemplate>
							<div class="kea-item-row" style="display:flex;align-items:center;justify-content:space-between;padding:10px 0;border-bottom:1px solid var(--dark-border);">
								<div class="item-details" style="display:flex;align-items:center;gap:12px;">
									<div>
										<div style="font-weight:600;font-size:14px;color:var(--light-text);"><%# Eval("ItemName") %></div>
										<div style="font-size:12px;color:var(--muted-text);">Qty: <%# Eval("Quantity") %></div>
									</div>
								</div>
					...
					<div style="display:flex;justify-content:space-between;padding-top:14px;font-weight:700;font-size:16px;color:var(--light-text);">
						<span>Total</span>
	...
	<script>
		...
		// DzCart is defined in Site.Master's own trailing script block, which renders AFTER this
		// content placeholder — so this ran before DzCart existed and silently no-opped every
		// time. Deferring to DOMContentLoaded guarantees DzCart is defined by the time this runs.
